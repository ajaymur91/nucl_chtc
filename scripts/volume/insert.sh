...
if [ "$#" -lt 1 ]; then
#	>&2 echo "Illegal number of parameters (requires atleast 1)"
#    	>&2 echo "Usage bash insert.sh   <gro file>   <Trials per cpu>   <ncpu>"
#	>&2 echo "Defaults bash insert.sh   <gro file>   100   \$(nproc)"
#	>&2 echo "\$(nproc) = $(nproc)"
    	exit 2
#else
#	echo "bash insert.sh   GRO=$GRO   Trials per cpu=$TRIALS   ncpu=$PROC"
fi
...
for i in `seq 0 $(($TRIALS-1))`; do
	for k in `seq 1 $PROC`; do
		{ gmx insert-molecules -f pad.gro -ci Na.gro -scale 0 -o gro/"$(($i*$PROC +$k))"-1.gro -nmol 1 &> /dev/null
	       	gmx insert-molecules -f gro/"$(($i*$PROC +$k))"-1.gro -ci $tempdir/Cl.gro -scale 0 -o gro/"$(($i*$PROC +$k))".gro -nmol 1 &> /dev/null
		echo "
